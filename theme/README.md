...
style:
  ...
  candidate_text_size: 22     #候选字体大小 默认值为：24
  candidate_view_height: 44   #候选高度 默认值为：44
  candidate_padding: 10       #候选字间距 默认值为：14
  comment_text_size: 8        #逐码显示的字体大小 默认值为：14
  comment_height: 16          #逐码显示的高度 默认值为：16
  show_preview: true          #是否显示按键提示气泡 默认显示 ，false
  round_corner: 0             # 圆角值为0的话默认 iphone为4，ipad为8
  keyboards: [default, number, .symbols, english, .number] #键盘配置,所有跳转键盘都需要在这里配置，不然会出错，导致键盘区不显示
